...
class BBCode
{
	...
	/**
     * spoiler_id of the spoiler class
     *
     * @var int
     */
	protected  $spoiler_id = 0;
    ...
    protected function generateTag(Tag $tag, &$html, Tag $openingTag = null, array $openTags = [])
    {
        ...
        switch ($tag->name) {
            ...
            case self::TAG_NAME_YOUTUBE:
                if ($tag->opening) {
                    ...
                    $code = '<iframe class="youtube-player" type="text/html" width="' . $this->youTubeWidth . '"\
                        height="' . $this->youTubeHeight . '" src="'.$ssl.'://www.youtube.com/embed/';
                } else {
            ...
            case self::TAG_NAME_SIZE:
                if ($tag->opening) {
                    if ($tag->property) {
                        $code = '<span style="font-size: '.$tag->property.'px">';
                    }
                } else {
                    $code = '</span>';
                }
            ...
            case self::TAG_NAME_SPOILER:
				$spoiler_id = $this->spoiler_id++;
                if ($tag->opening) {
                    $code = '<div class="spoiler"><input type="checkbox" id="spoilerid_'.$spoiler_id.'"><label for="spoilerid_'.$spoiler_id.'"> ספויילר </label><div class="spoiler_body">';
                } else {
                    $code = '</div></div>';
                }
                break;
    ...
    public function replace_smily($msg) {
        $smiles = array(
          '8-)' => 'shades',
          'o:)' => 'angel',
          '>:(' => 'angry',
        ...
        foreach($smiles as $key => $img) {
        	$msg = str_replace($key, '<img src="./assest/framework/ckeditor/plugins/smiley/images/'.$img.'_smile.png" height="20" width="20" />', $msg);
        }
        return $msg;
    }
